...
const {insertSpamComment, insertGoodComment} = require('./Database.js');
...
class Staticman {
  ...
  _checkForSpam (options, fields) {
    ...
    return new Promise((resolve, reject) => {
      ...
      akismet.checkSpam(comment, (err, isSpam) => {
        if (err) return reject(err)

        if (isSpam){
            insertSpamComment({...comment, site: config.get('akismet.site')}); 
            return reject(errorHandler('IS_SPAM'))
        } 

        insertGoodComment({...comment, site: config.get('akismet.site')}); 
  ...
  processEntry (fields, options) {
    ...
    return this.getSiteConfig().then(config => {
      ...
      return this._checkForSpam(options, fields)
    }).then(fields => {
      // Validate fields
      ...
      const commitMessage = this._resolvePlaceholders(this.siteConfig.get('commitMessage'), {
        ...
        // Hack to make sure mailing works right.
        // We need to make sure that a top-level comment
        // subscribes to its own mailing list.
        if (!options.parent) {
  ...
  processMerge (fields, options) {
    ...
    return this.getSiteConfig().then(config => {
      const subscriptions = this._initialiseSubscriptions()
      return subscriptions.send(fields.parent, fields, options, this.siteConfig)
    }).catch(err => {
